...
export const allModes: ModeConfig[] = [
  ...
  },
  {
    ...
    icon: "🚗",
    category: "play",
    description: "Road trip games and travel fun",
  ...
  },
  {
    ...
    icon: "🌙",
    category: "support",
    description: "Gentle songs to help you fall asleep",
...
/**
 * UI-focused mode registry for mobile and web-revamp.
 *
 * Unlike `allModes` (which uses emoji icons and snake_case slugs for the landing
 * app), `modeConfigs` uses Lucide icon names, kebab-case slugs, and includes
 * colour tokens used by the companion UI.
 */
...
export const modeConfigMap: Record<string, ModeConfig> = Object.fromEntries(
  modeConfigs.map((m) => [m.slug, m])
);

export function getModeConfig(slug: string): ModeConfig | undefined {
  return modeConfigMap[slug];
}
