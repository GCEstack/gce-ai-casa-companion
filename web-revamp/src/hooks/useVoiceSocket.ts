...
function getOrCreateDeviceId(): string {
  try {
    const existing = sessionStorage.getItem('casa_web_device_id');
    if (existing) return existing;
    const id = crypto.randomUUID();
    sessionStorage.setItem('casa_web_device_id', id);
    return id;
  } catch {
...
function getOrCreateSessionId(): string {
  try {
    const existing = sessionStorage.getItem('casa_web_session_id');
    if (existing) return existing;
    const id = crypto.randomUUID();
    sessionStorage.setItem('casa_web_session_id', id);
    return id;
  } catch {
